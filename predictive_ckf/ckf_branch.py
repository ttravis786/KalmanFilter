...
class CKF_branch():
    def __init__(self, X, P, curr_cell, prev_cell,
                 points, total_points, ckf, master=None):
        self.X = X
        self.P = P
    ...
    def resid_func(self, max_res):
        return max_res / 3
        #return max_res
        # if self.total_points > 20:
        #     return max_res/3
        # return max_res * np.exp(-(self.total_points-5)/(30))
    ...
    def propogate(self, max_range, points_map, max_res, try_all=False):
        # if self.total_points == 28 and np.all(self.points[-1] == np.array([2.47719008, -10.3427758 ])):
        #     a = 1
            ...
            for i in range(1, max_range):
                ...
                for cell in next_cells:
                    ...
                    for next_point in points_map[cell]:
                        ...
                        X, P, y_k_k = self._update_kf(next_point[0], next_point[1])
                        if np.any([np.all(next_point == p) for p in self.full_points()]):
                            continue
                        if y_k_k[0] < self.resid_func(max_res):
                ...
                if len(possible_next_points) == 1:
                    ...
                    for p in possible_next_points:
                        ...
                        self.children.append(CKF_branch(X, P, cell, self.curr_cell, [next_point],
                                                        self.total_points + 1, self.ckf, master=self))
                    for c in self.children:
                        #points_map_n = copy.deepcopy(points_map)
